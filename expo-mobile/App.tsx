      ...
      <View style={s.content}>
        ...
        {activeTab === 'telemetry' && (
          <ScrollView style={s.tabScroll} contentContainerStyle={{ padding: 16, paddingBottom: 36 }}>
            ...
            {/* Real-time Hardware Simulator Engine Controls & Telemetry Monitor */}
            {isCharging ? (
              <View style={s.simPanelCard}>
                <View style={s.simPanelHeader}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                    <Zap size={14} color="#f59e0b" />
                    <Text style={s.simPanelTitle}>HARDWARE SIMULATOR & TUNING</Text>
                  </View>
                  <View style={s.simLiveBadge}>
                    <View style={s.activePulseDot} />
                    <Text style={s.simLiveBadgeText}>STREAMING {simSpeed}x</Text>
                  </View>
                </View>

                {/* Speed Multiplier */}
                <Text style={s.simSubLabel}>SIMULATION CLOCK RATE</Text>
                <View style={s.simChipRow}>
                  {[
                    { mult: 1, label: '1x (Real-time)' },
                    { mult: 5, label: '5x (Fast)' },
                    { mult: 10, label: '10x (Turbo)' },
                  ].map((item) => (
                    <TouchableOpacity
                      key={item.mult}
                      style={[s.simChip, simSpeed === item.mult && s.simChipActive]}
                      onPress={() => handleSetSimSpeed(item.mult)}
                    >
                      <Text style={[s.simChipText, simSpeed === item.mult && s.simChipTextActive]}>
                        {item.label}
                      </Text>
                    </TouchableOpacity>
                  ))}
                ...
                <Text style={[s.simSubLabel, { marginTop: 10 }]}>THROTTLE SIMULATED POWER (kW)</Text>
                <View style={s.simChipRow}>
                  {[50, 120, 160, 240].map((kw) => (
                    <TouchableOpacity
                      key={kw}
                      style={[s.simChip, Math.round(currentKw) === kw && s.simChipActive]}
                      onPress={() => handleSetSimPower(kw)}
                    >
                      <Text style={[s.simChipText, Math.round(currentKw) === kw && s.simChipTextActive]}>
                        {kw} kW
                      </Text>
                    </TouchableOpacity>
                  ))}
                ...
                {/* Live OCPP 2.0.1 MeterValues Stream */}
                <Text style={[s.simSubLabel, { marginTop: 12 }]}>LIVE OCPP 2.0.1 METERVALUES TELEMETRY</Text>
                <View style={s.ocppAuditBox}>
                  {recentOcppPackets.slice(0, 3).map((pkt) => (
                    <View key={pkt.id} style={s.ocppRow}>
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                        <Text style={s.ocppTime}>{pkt.time}</Text>
                        <Text style={s.ocppAction}>[{pkt.action}]</Text>
                      </View>
                      <Text style={s.ocppSummary}>{pkt.summary}</Text>
                    </View>
                  ))}
                </View>
              </View>
              ...
              <View style={s.simPanelCard}>
                <View style={s.simPanelHeader}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                    ...
                    <Text style={[s.simPanelTitle, { color: '#38bdf8' }]}>
                      REAL-TIME STATION HARDWARE SIMULATOR
                    </Text>
                  </View>
                  <View style={[s.simLiveBadge, { backgroundColor: 'rgba(56,189,248,0.1)', borderColor: 'rgba(56,189,248,0.3)' }]}>
                    <Text style={[s.simLiveBadgeText, { color: '#38bdf8' }]}>STANDBY</Text>
                  </View>
                </View>
                <Text style={s.simExplainerText}>
                  Test live OCPP 2.0.1 charging telemetry, battery charging curves, and Ghana MoMo pre-auth billing without requiring a physical EV charging station.
                </Text>

                {/* Station Selection */}
                <Text style={s.simSubLabel}>SELECT SIMULATED CHARGER</Text>
                <View style={s.simChipRow}>
                  {stations.map((st) => (
                    <TouchableOpacity
                      key={st.stationId}
                      style={[s.simChip, simStationId === st.stationId && s.simChipActive]}
                      onPress={() => setSimStationId(st.stationId)}
                    >
                      <Text style={[s.simChipText, simStationId === st.stationId && s.simChipTextActive]}>
                        {st.name.replace('XCharge ', '').split('–')[0].trim()}
                      </Text>
                    </TouchableOpacity>
                  ))}
                </View>
                ...
                <Text style={[s.simSubLabel, { marginTop: 10 }]}>SIMULATION SPEED</Text>
                <View style={s.simChipRow}>
                  {[
                    { mult: 1, label: '1x (Real-time)' },
                    { mult: 5, label: '5x (Fast Test)' },
                    { mult: 10, label: '10x (Turbo Demo)' },
                  ].map((item) => (
                    <TouchableOpacity
                      key={item.mult}
                      style={[s.simChip, simSpeed === item.mult && s.simChipActive]}
                      onPress={() => setSimSpeed(item.mult)}
                    >
                      <Text style={[s.simChipText, simSpeed === item.mult && s.simChipTextActive]}>
                        {item.label}
                      </Text>
                ...
                {/* Initial SoC */}
                <Text style={[s.simSubLabel, { marginTop: 10 }]}>INITIAL PACK STATE OF CHARGE</Text>
                <View style={s.simChipRow}>
                  {[
                    { soc: 15, label: '15% (Low)' },
                    { soc: 28, label: '28% (Typical)' },
                    { soc: 52, label: '52% (Mid)' },
                  ].map((item) => (
                    <TouchableOpacity
                      key={item.soc}
                      style={[s.simChip, simInitialSoc === item.soc && s.simChipActive]}
                      onPress={() => setSimInitialSoc(item.soc)}
                    >
                      <Text style={[s.simChipText, simInitialSoc === item.soc && s.simChipTextActive]}>
                        {item.label}
                      </Text>
                    </TouchableOpacity>
                  ))}
                ...
                <TouchableOpacity
                  style={s.simLaunchBtn}
                  onPress={() => handleLaunchSimulatedSession()}
                  activeOpacity={0.85}
                >
                  <Play size={16} color="#020817" style={{ marginRight: 6 }} />
                  <Text style={s.simLaunchBtnText}>LAUNCH SIMULATED CHARGE & STREAM DATA</Text>
                </TouchableOpacity>
              </View>
            )}
