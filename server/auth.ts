...
function loadUsersFromDisk(): Map<string, UserProfile> {
  ...
  const defaultDriver: UserProfile = {
    ...
    email: 'kofi.mensah@chargelink.com.gh',
...
export async function sendOtp(phoneNumber: string): Promise<{ success: boolean; message: string; devCode?: string }> {
  ...
  if (moolreVasKey) {
    try {
      ...
      // Moolre error reporting (e.g. ASMS06 bundle exhausted)
...
export function verifyOtp(
  ...
  if (!user) {
    ...
    user = {
      ...
      email: metadata?.email || `${normalized.replace(/\D/g, '')}@chargelink.com.gh`,
...
export function creditUserWallet(
  ...
  if (!user) {
    user = {
      ...
      email: `${norm.replace(/\D/g, '')}@chargelink.com.gh`,
      walletBalance: 0,
      heldEscrow: 0,
